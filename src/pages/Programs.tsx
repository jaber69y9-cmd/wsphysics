    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          setFormData({ name: '', email: '', password: '', confirmPassword: '', phone: '', current_class: '', school_name: '', college_name: '', batch_id: '' });
        }, 3000);
      } else {
                    ...
                    <div className="p-8 md:p-12 space-y-8">
                      <div>
                        ...
                        <div className="relative">
                          <div className={`text-slate-600 leading-relaxed ${!selectedProgramIdForDesc.includes(course.id) ? 'line-clamp-3' : ''}`}>
                            {course.description}
                          </div>
                          {course.description && course.description.length > 150 && (
                            <button 
                              ...
                              className="text-orange-600 text-[10px] font-black uppercase tracking-widest hover:underline mt-2 block"
                            >
                  ...
                  <form onSubmit={handleEnroll} className="p-6 space-y-4 overflow-y-auto max-h-[70vh] scrollbar-hide">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      <div>
                        ...
                        <select 
                          required
                          className="w-full border border-slate-200 rounded-xl px-4 py-3 text-slate-800 outline-none focus:border-orange-500 glass-input"
                          value={formData.batch_id}
                          onChange={(e) => setFormData({...formData, batch_id: e.target.value})}
                        >
                          <option value="">Select a batch...</option>
                          {batches.filter(b => String(b.program_id) === String(selectedProgram.id)).map(batch => (
                            <option key={batch.id} value={batch.id}>{batch.name} ({batch.days} - {batch.time})</option>
                          ))}
                        </select>
                      </div>
                    </div>

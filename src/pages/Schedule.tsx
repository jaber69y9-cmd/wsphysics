...
const Schedule = () => {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/web-class-schedules').then(res => res.ok ? res.json() : [])
          ...
          <div className="space-y-24">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                {webSchedules.map((sch) => (
                  <motion.div 
                    ...
                    viewport={{ once: true }}
                    className="bg-white p-4 rounded-[2.5rem] shadow-2xl border border-orange-100 overflow-hidden"
                  >
                    <h3 className="text-2xl font-bold text-slate-800 mb-6 px-4">{sch.title}</h3>
                    <div className="relative group">
                      <img src={sch.image_url} alt={sch.title} className="w-full rounded-2xl shadow-inner" />
            ...
            {Array.from(new Set(batches.map(b => b.class))).sort().map((category) => (
              <div key={category} className="space-y-8">
                ...
                <div className="bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100/50 backdrop-blur-sm">
            ...
            {batches.length === 0 && webSchedules.length === 0 && !loading && (

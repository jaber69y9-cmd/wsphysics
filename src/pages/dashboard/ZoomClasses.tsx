    ...
    try {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data)) {
          // Filter by student's enrolled courses if online student
          if (user?.student_type === 'online' && user?.enrolled_courses) {
            const enrolledCourseIds = user.enrolled_courses.map((c: any) => c.id);
            const filtered = data.filter((lc: any) => 
